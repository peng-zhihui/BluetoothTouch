        ...
        mRunnable = new Runnable()
        {
            ...
            public void run()
            {
                ...
                if (MainActivity.mChatService == null)
                    return;
                if (MainActivity.mChatService.getState() == BluetoothChatService.STATE_CONNECTED && MainActivity.checkTab(ViewPagerAdapter.JOYSTICK_FRAGMENT))
                {
                    if (!getResources().getBoolean(R.bool.isTablet) || !MainActivity.buttonState)
                    { // Don't send stop if the button in the IMU fragment is pressed
